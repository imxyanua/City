...
void applyPreset(UIPreset preset, AppOptions& opts, Scene& scene, Camera& camera) {
    ...
    if (preset == UIPreset::Day) {
        ...
        opts.timeOfDayHour = 13.2f;
        opts.rainIntensity = 0.0f;
        opts.windDir = glm::normalize(glm::vec2(-0.55f, -0.12f));
        opts.cloudHeight = 125.0f;
        scene.setFogDensity(0.015f);
        scene.setFogBaseHeight(0.0f);
        scene.setFogHeightFalloff(0.025f);
        scene.setFogColor(glm::vec3(0.52f, 0.59f, 0.70f));
        scene.setExposure(1.38f);
        scene.setWindowEmissive(0.9f);
        opts.bloomIntensity = 0.55f;
        opts.godRayIntensity = 0.75f;
        opts.colorGradeIntensity = 0.55f;
        opts.vignetteIntensity = 0.28f;
        opts.ssrIntensity = 0.18f;
    }
    if (preset == UIPreset::Night) {
        opts.syncTimeOfDay = false;
        opts.timeOfDayHour = 21.3f;
        opts.rainIntensity = 0.05f;
        opts.windDir = glm::normalize(glm::vec2(-0.45f, -0.2f));
        opts.cloudHeight = 105.0f;
        scene.setFogDensity(0.12f);
        scene.setFogBaseHeight(0.0f);
        scene.setFogHeightFalloff(0.05f);
        scene.setFogColor(glm::vec3(0.18f, 0.22f, 0.30f));
        scene.setExposure(0.78f);
        scene.setWindowEmissive(3.4f);
        opts.bloomIntensity = 0.8f;
        opts.godRayIntensity = 0.12f;
        opts.colorGradeIntensity = 0.85f;
        opts.vignetteIntensity = 0.82f;
        opts.ssrIntensity = 0.45f;
    }
    if (preset == UIPreset::Rain) {
        opts.syncTimeOfDay = false;
        opts.timeOfDayHour = 18.2f;
        opts.rainIntensity = 0.9f;
        opts.windDir = glm::normalize(glm::vec2(-1.0f, -0.4f));
        opts.cloudHeight = 85.0f;
        scene.setWetness(opts.rainIntensity);
        scene.setFogDensity(0.45f);
        scene.setFogBaseHeight(-1.5f);
        scene.setFogHeightFalloff(0.10f);
        scene.setFogColor(glm::vec3(0.24f, 0.30f, 0.40f));
        scene.setExposure(0.98f);
        scene.setWindowEmissive(2.5f);
        opts.bloomIntensity = 0.30f;
        opts.godRayIntensity = 0.05f;
        opts.colorGradeIntensity = 0.95f;
        opts.vignetteIntensity = 0.78f;
        opts.ssrIntensity = 1.25f;
        opts.ssaoIntensity = 1.35f;
    }
    if (preset == UIPreset::Sunny) {
        opts.syncTimeOfDay = false;
        opts.timeOfDayHour = 9.6f;
        opts.rainIntensity = 0.0f;
        opts.windDir = glm::normalize(glm::vec2(-0.35f, -0.08f));
        opts.cloudHeight = 135.0f;
        scene.setFogDensity(0.0f);
        scene.setFogBaseHeight(0.0f);
        scene.setFogHeightFalloff(0.02f);
        scene.setFogColor(glm::vec3(0.58f, 0.63f, 0.72f));
        scene.setExposure(1.48f);
        scene.setWindowEmissive(0.7f);
        opts.bloomIntensity = 0.85f;
        opts.godRayIntensity = 1.0f;
        opts.colorGradeIntensity = 0.45f;
        opts.vignetteIntensity = 0.2f;
        opts.ssrIntensity = 0.15f;
    }
    if (preset == UIPreset::Noir) {
        opts.syncTimeOfDay = false;
        opts.timeOfDayHour = 19.6f;
        opts.rainIntensity = 0.25f;
        opts.windDir = glm::normalize(glm::vec2(-0.7f, -0.3f));
        opts.cloudHeight = 90.0f;
        scene.setWetness(opts.rainIntensity);
        scene.setFogDensity(0.22f);
        scene.setFogBaseHeight(0.0f);
        scene.setFogHeightFalloff(0.07f);
        scene.setFogColor(glm::vec3(0.15f, 0.18f, 0.24f));
        scene.setExposure(0.82f);
        scene.setWindowEmissive(2.8f);
        opts.bloomIntensity = 0.2f;
        opts.godRayIntensity = 0.08f;
        opts.colorGradeIntensity = 1.0f;
        opts.vignetteIntensity = 0.95f;
        opts.chromaticAberration = 0.0f;
        opts.ssrIntensity = 0.6f;
        opts.ssaoIntensity = 1.8f;
    }
}
