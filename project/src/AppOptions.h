...
struct AppOptions {
    ...
    /// Mảnh thống kê xe / pedestrian / khối thành phố trong overlay góc
...
inline void saveConfig(const AppOptions& opts, const std::string& path) {
    ...
    j["showDebugStats"] = opts.showDebugStats;
...
inline void loadConfig(AppOptions& opts, const std::string& path) {
    ...
    try {
        ...
        if (j.contains("showDebugStats")) opts.showDebugStats = j["showDebugStats"];
